...
const DamagedPage = ({ navigate }) => {
    const collectionName = 'damaged';
    // Define the fields specific to the Damaged form
    const modalFields = ['Model', 'Serial Number', 'Category', 'Laptop warrenty', 'Lifespan', 'Damage Description', 'Location'];
    ...
    const filterOptions = useMemo(() => {
        const options = { Model: new Set(), Location: new Set(), Category: new Set() };
        assets.forEach(asset => {
            Object.keys(options).forEach(key => {
                if (asset[key]) options[key].add(asset[key]);
            });
        });
        return { Model: [...options.Model], Location: [...options.Location], Category: [...options.Category] };
    }, [assets]);
    ...
    const handleDownload = () => {
        ...
        link.href = url;
        link.setAttribute('download', 'damaged_assets_data.csv');
    ...
    return (
        <PageShell pageTitle="Damaged Assets" navigate={navigate}>
