...
const EWastePage = ({ navigate }) => {
    const collectionName = 'ewaste';
    // Define the fields specific to the E-Waste form
    const modalFields = ['Asset ID', 'Date of Move', 'Approved By', 'Reason for Disposal'];
    ...
    const [filters, setFilters] = useState({ 'Approved By': '', 'Reason for Disposal': '' });
    ...
    const filterOptions = useMemo(() => {
        const options = { 'Approved By': new Set(), 'Reason for Disposal': new Set() };
        assets.forEach(asset => {
            Object.keys(options).forEach(key => {
                if (asset[key]) options[key].add(asset[key]);
            });
        });
        return { 'Approved By': [...options['Approved By']], 'Reason for Disposal': [...options['Reason for Disposal']] };
    }, [assets]);
    ...
    const clearFilters = () => {
        setFilters({ 'Approved By': '', 'Reason for Disposal': '' });
    };
    ...
    const handleDownload = () => {
        ...
        link.href = url;
        link.setAttribute('download', 'ewaste_assets_data.csv');
    ...
    return (
        <PageShell pageTitle="E-Waste Assets" navigate={navigate}>
