...
// Helper icons for the control bar
...
const MasterPage = ({ navigate }) => {
    ...
    const modalFields = ['Assignee Name', 'Postion', 'Employee Email', 'License Type', 'Phone Number', 'Department', 'Model', 'Serial Number', 'Comment', 'Location', 'laptop warrenty and validity', 'Lifespan of laptop', 'Status'];
    ...
    const [filters, setFilters] = useState({ Model: '', Location: '', Status: '', Department: '' });
    ...
    const filterOptions = useMemo(() => {
        const options = { Model: new Set(), Location: new Set(), Status: new Set(), Department: new Set() };
        assets.forEach(asset => {
            Object.keys(options).forEach(key => {
                if (asset[key]) options[key].add(asset[key]);
            });
        });
        return { Model: [...options.Model], Location: [...options.Location], Status: [...options.Status], Department: [...options.Department] };
    }, [assets]);
    ...
    const clearFilters = () => {
        setFilters({ Model: '', Location: '', Status: '', Department: '' });
    };
    ...
    const handleDownload = () => {
        ...
        link.href = url;
        link.setAttribute('download', 'master_view_data.csv');
    ...
    return (
        <PageShell pageTitle="Master View" navigate={navigate}>
